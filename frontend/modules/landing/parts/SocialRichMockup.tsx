...
export const SocialRichMockup: React.FC<{ platform: SocialPlatform }> = ({ platform }) => {
  if (platform === "x") {
    return (
      <div className="bg-[#000000] p-3.5 rounded-2xl border border-white/10 mb-4 text-white text-xs">
        <div className="flex items-center gap-1.5 mb-1.5">
          <span className="font-bold text-white">@CreatorDeals</span>
          <span className="h-3 w-3 rounded-full bg-sky-500 text-white flex items-center justify-center text-[8px] font-bold">✓</span>
          <span className="text-zinc-500">&middot; 2m</span>
        </div>
        ...
        <div className="rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900">
          <div className="relative h-28">
            <img src="/api/preview-image" alt="Sony Earbuds" className="w-full h-full object-cover" />
          ...
          <div className="p-2 text-[10.5px]">
            <div className="text-zinc-400 font-mono text-[9px]">smartlink.to</div>
            <div className="font-bold text-zinc-100 truncate">Sony Wireless Noise-Cancelling Earbuds</div>
          </div>
        </div>
      </div>
    );
    ...
    <div className={`p-3.5 rounded-2xl border border-white/10 mb-4 ${isTg ? "bg-[#0e1621]" : "bg-[#0c1317]"}`}>
      <div className={`p-3 rounded-xl border space-y-2 max-w-sm text-white shadow-lg ${isTg ? "bg-[#182533] border-[#229ED9]/30" : "bg-[#005c4b] border-emerald-400/20"}`}>
        <div className="relative h-28 rounded-lg overflow-hidden border border-white/15">
          <img src="/api/preview-image" alt="Sony Earbuds" className="w-full h-full object-cover" />
          <span className="absolute top-2 left-2 bg-red-600 text-white font-bold text-[9px] px-1.5 py-0.5 rounded">🔥 60% OFF</span>
          <span className="absolute bottom-2 right-2 bg-black/80 text-[#ccff00] text-[8.5px] font-mono px-1.5 py-0.5 rounded flex items-center gap-1">
            <AmazonLogo className="h-2.5 w-2.5" /> Amazon App
          </span>
        </div>
